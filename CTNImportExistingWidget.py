...
""" Widget for the existing values in .csv format"""


class CTNImportExistingWidget(QtWidgets.QWidget):
    """ Constructor """
    ...
    def import_existing_view(self):
        try:
            ...
            rows_ = ctn_db.read_table_db(type_=self.ex_tp, proj_name=self.project_name, proj_revision=self.revision_name)
            ...
            self.current_line.setText('Something get wrong! Check the existing DB...')
    ...
    def save_file(self):
        try:
            if len(self.file_name) > 0:
                ...
                text_ = 'Existing points file is saved to the project: ' + self.project_name + '; rev: ' + self.revision_name
                self.current_line.setText(text_)
            else:
                text_ = "No any project is open!\n"
                self.current_line.setText(text_)
    ...
    """ Import the existing values file """
    ...
    def import_file(self):
        ...
        if ok_select:
            try:
                if file_name[-3:] == 'csv':
                    if self.project_open:
                        ctn_db.import_file(file_dir=file_name, type_=self.ex_tp, proj_name=self.project_name,
                ...
                self.current_line.setText('Something get wrong! Check the csv file or DB')
        self.import_existing_view()
            ...
            for i in range(len(repeat_list)):
                for j in range(len(self.point_list)):
                    if repeat_list[i] == self.point_list[j]:
                        ...
                        break
            ctn_db.delete_one_row(id_del=id_del, type_=self.ex_tp, proj_name=self.project_name,
    ...
    def delete_all(self):
        ...
        text_sel, ok_pressed = QtWidgets.QInputDialog.getItem(self, 'Select', 'Yes or No', text_types, 0, False)
        if ok_pressed and text_sel == 'Yes':
            ctn_db.delete_all(type_=self.ex_tp, proj_name=self.project_name, proj_revision=self.revision_name)
