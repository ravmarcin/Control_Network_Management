...
""" Widget to show the changed points from analysis of residuals """


class CTNAnalysisChangePointsWidget(QtWidgets.QWidget):
    """ Constructor """
    ...
    def analysis_change_p_view(self):
        ...
        rows_ = ctn_db.read_table_db(type_=self.ch_tp, proj_name=self.project_name, proj_revision=self.revision_name)
            ...
            for i in range(len(repeat_list)):
                for j in range(len(self.point_list)):
                    if repeat_list[i] == self.point_list[j]:
                        ...
                        break
            ctn_db.delete_one_row(id_del=id_del, type_=self.ch_tp, proj_name=self.project_name,
    ...
    def delete_one_point(self):
        ...
        if len(to_choose) > 0:
            ...
            name_del, ok_pressed = QtWidgets.QInputDialog.getItem(self, 'Select', 'Point to delete', to_choose, 0,
                                                                  False)
            for j in range(len(self.point_list)):
    ...
    def delete_all(self):
        ...
        text_sel, ok_pressed = QtWidgets.QInputDialog.getItem(self, 'Select', 'Yes or No', text_types, 0, False)
        if ok_pressed and text_sel == 'Yes':
            ctn_db.delete_all(type_=self.ch_tp, proj_name=self.project_name, proj_revision=self.revision_name)
