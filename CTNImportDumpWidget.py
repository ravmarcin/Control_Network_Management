...
""" Widget for import the dump files in .csv format"""


class CTNImportDumpWidget(QtWidgets.QWidget):
    """ Constructor """
    ...
    def import_dump_view(self):
        table_info = ctn_db.read_col_db(type_=self.dp_tp, proj_name=self.project_name, proj_revision=self.revision_name)
        col_names = [col_[1] for col_ in table_info]
        rows_ = ctn_db.read_table_db(type_=self.dp_tp, proj_name=self.project_name, proj_revision=self.revision_name)
        ...
        self.grid.addWidget(btn_import, 1, 7, 1, 1)
    ...
    def save_file(self):
        if len(self.file_name) > 0:
            ctn_db.save_to_project(proj_name=self.project_name, proj_revision=self.revision_name, type_=self.dp_tp)
            text_ = 'Dump file is saved to the project: ' + self.project_name + '; rev: ' + self.revision_name
            self.current_line.setText(text_)
        else:
            text_ = "No any project is open!\n"
            self.current_line.setText(text_)
    ...
    def import_file(self):
        ...
        if ok_select:
            if file_name[-3:] == 'csv':
                if self.project_open:
                    ctn_db.import_file(file_dir=file_name, type_=self.dp_tp, proj_name=self.project_name,
            ...
            for i in range(len(repeat_list)):
                for j in range(len(self.point_list)):
                    if repeat_list[i] == self.point_list[j]:
                        ...
                        break
            ctn_db.delete_one_row(id_del=id_del, type_=self.dp_tp, proj_name=self.project_name,
    ...
    def delete_all(self):
        ...
        text_sel, ok_pressed = QtWidgets.QInputDialog.getItem(self, 'Select', 'Yes or No', text_types, 0, False)
        if ok_pressed and text_sel == 'Yes':
            ctn_db.delete_all(type_=self.dp_tp, proj_name=self.project_name, proj_revision=self.revision_name)
